...
export const useMapStore = defineStore("map", () => {
  ...
  function onMapObjectLock(prevLockedMapObject: MapDataObject | undefined) {
    ...
    mapMarkers.value.forEach((marker: Marker) => {
      ...
      if (
        marker.getLngLat().lat == lockedLatitude &&
        marker.getLngLat().lng == lockedLongitude
      ) {
    ...
    if (
      prevLockedMapObject?.position?.latitude ==
        lockedMapObject.value.position.latitude &&
      prevLockedMapObject?.position?.longitude ==
        lockedMapObject.value.position.longitude
    ) {
      return;
    }
  ...
  function centerLockedMapObject() {
    mapData.value.forEach((mapDataObject: MapDataObject) => {
      if (
        !mapInstance.value ||
        !mapDataObject.position ||
        !lockedMapObject.value ||
        !lockedMapObject.value.position
      ) {
        return;
      }

      if (
        mapDataObject.position.id == lockedMapObject.value.position.id ||
        mapDataObject.identifier !== lockedMapObject.value.identifier
      ) {
        return;
      }
      ...
      if (
        currentLatitude == lockedLatitude &&
        currentLongitude == lockedLongitude
      ) {
        return;
      }
  ...
  const filteredMapData = computed(() => {
    return mapData.value.filter((mapDataObject: MapDataObject) => {
      return mapDataObject.identifier
        .toLowerCase()
        .trim()
        .includes(mapSearchQuery.value.toLowerCase().trim());
    });
  });
