  ...
  <article class="modal__box" :class="size">
    <section class="box__top">
      ...
      <p class="box__subtitle">
        Link a taxi identifier to allow position updates to be sent to the
        server.
      </p>
    </section>
    <form action="" class="box__form">
      <div
        v-if="formErrorStore.error"
        class="form__error"
        :class="{ 'error--success': formErrorStore.error.success }">
        {{ formErrorStore.error.message }}
      </div>
    ...
    <article class="box__actions">
      <button @click="modalStore.closeModal" class="button__secondary">
        Cancel
      </button>
...
function addNewTaxi() {
  ...
  //   if (error) {
  //     formErrorStore.setMessage(error.message);
  //     return;
  //   }
